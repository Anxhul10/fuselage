...
async function run() {
  ...
  for (const pkg of pkgs) {
    if (regex[`${pkg}`].length === 0) {
      ...
      await execCommand(`cd packages/${pkg} && yarn loki test --chromeDockerImage=zenika/alpine-chrome --chromeFlags='--headless --no-sandbox --disable-gpu --disable-features=VizDisplayCompositor' --verboseRenderer`);
    } else {
      console.log(colorize(`Running fuselageSnap for ${pkg}`).bgBlue);
      await execCommand(
        `cd packages/${pkg} && yarn loki test --chromeDockerImage=zenika/alpine-chrome --chromeFlags='--headless --no-sandbox --disable-gpu --disable-features=VizDisplayCompositor' --verboseRenderer --storiesFilter="${regex[pkg]}"`,
      );
    }
  }
}
